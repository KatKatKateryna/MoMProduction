#!/usr/bin/env python3
"""
Checks GitHub hourly for a new Final_Attributes CSV.
If found: joins with watershed shapefile, regenerates data/watersheds.pmtiles.

Usage:
    python update_tiles.py          # run every hour indefinitely
    python update_tiles.py --once   # run once and exit
    python update_tiles.py --interval 1800  # run every 30 minutes
"""
...
STATE_FILE  = OUT_DIR / '.last_csv'

# ── Config ────────────────────────────────────────────────────────────────────

GITHUB_API  = 'https://api.github.com/repos/Global-Flood-Assessment/MoMOutputStream/contents/ModelOutputs'
...
def fetch_latest_csv_info():
    r = requests.get(GITHUB_API, headers={'Accept': 'application/vnd.github.v3+json'}, timeout=30)
    r.raise_for_status()
    files = sorted(
        [f for f in r.json() if f['name'].startswith('Final_Attributes_') and f['name'].endswith('.csv')],
        key=lambda f: f['name'],
        reverse=True,
    )
...
def process_csv(df):
    ...
    for pfaf, rows in groups.items():
        ...
        countries = ', '.join(sorted({str(r['name']) for r in rows if pd.notna(r.get('name'))}))
        regions   = ', '.join(sorted({str(r['name_1']) for r in rows if pd.notna(r.get('name_1'))}))
        records.append({
            ...
            'days_until_peak': int(base['Days_until_peak']) if pd.notna(base.get('Days_until_peak')) else None,
        })
...
def regenerate_pmtiles(alert_df, csv_name):
    ...
    if ogr.GetDriverByName('PMTiles') is None:
        print('ERROR: GDAL PMTiles driver not available. Ensure GDAL >= 3.7 is installed.')
        sys.exit(1)
    ...
    if result is None:
        raise RuntimeError('gdal.VectorTranslate failed — check GDAL error output above')
...
def run_once():
    ...
    try:
        ...
        STATE_FILE.write_text(info['name'])
    except Exception as e:
        print(f'  Error: {e}')
        raise
...
if __name__ == '__main__':
    ...
    parser.add_argument('--interval', type=int, default=1800, help='Check interval in seconds (default: 1800)')
