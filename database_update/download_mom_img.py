...
LOCAL_ROOT = Path(__file__).parent / "downloads_img" if not Path("/mnt").exists() else Path("/mnt/temp_img_download/downloads_img") 
LIMIT = None
FILE_END = ".tiff"
...
def crawl(url, local_dir, session):
    ...
    if FILE_END:
        files = [f for f in files if f.lower().endswith(FILE_END)]
