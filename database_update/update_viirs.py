"""
Incrementally update summary_viirs_all.csv with new VIIRS summary files.

1. Read existing timestamps from summary_viirs_all.csv.
...
3. For each new file (in sorted order):
   ...
   b. Append all rows to summary_viirs_all.csv.
"""
...
OUTPUT_FILE = os.path.join(base_dir, "summary_viirs_all.csv")
...
def extract_rows(content, timestamp):
    ...
    for row in reader:
        rows.append({
            "timestamp":             timestamp,
            "pfaf_id":               row["pfaf_id"],
            "onedayFlood_Area_km":   row["onedayFlood_Area_km"],
            "onedayperc_Area":       row["onedayperc_Area"],
            "fivedayFlood_Area_km":  row["fivedayFlood_Area_km"],
            "fivedayperc_Area":      row["fivedayperc_Area"],
        })
    return rows
